  ...
  effect: Effect.gen(function* () {
    ...
    const createConversation = (conversationId: string) =>
      Effect.gen(function* () {
        ...
        yield* Effect.tryPromise({
          try: () => fs.writeFile(path, JSON.stringify(conversation, null, 2)),
    ...
    const listConversations = Effect.gen(function* () {
      ...
      for (const filename of files) {
        if (filename.endsWith(".json")) {
          ...
          const conversation = yield* Effect.either(getConversation(filename.replace(".json", "")));
...
export const addAssistantMessage = (
  ...
  Effect.runSync(
    Effect.gen(function* () {
      const storage = yield* StorageService;
      return yield* storage.addAssistantMessage(conversationId, stage1, stage2, stage3);
    }).pipe(Effect.provide(StorageServiceLive))
  );
