...
import { StorageService, type Stage1Response, type Stage2Response, type Stage3Response } from "./storage";
 ...
 */
export class CouncilService extends Effect.Service<CouncilService>()("CouncilService", {
    effect: Effect.gen(function* () {
        ...
        const stage1CollectResponses = (userQuery: string) =>
            Effect.gen(function* () {
                ...
                const responses = yield* openRouter.queryModelsParallel(config.councilModels, messages);
                ...
                if (stage1Results.length === 0) {
                    return yield* Effect.fail(new CouncilError({
                        stage: 1,
         ...
         */
        const stage2CollectRankings = (userQuery: string, stage1Results: Stage1Response[]) =>
            Effect.gen(function* () {
...
Provide a clear, well-reasoned final answer that represents the council's collective wisdom:`;
                ...
                const response = yield* openRouter.queryModel(config.chairmanModel, messages);
        ...
        const parseRankingFromText = (rankingText: string): string[] => {
            ...
            if (rankingText.includes("FINAL RANKING:")) {
                ...
                if (parts.length >= 2) {
                    ...
                    const numberedMatches = rankingSection.match(NUMBERED_RESPONSE_PATTERN);
...
Title:`;
                ...
                const response = yield* openRouter.queryModel("google/gemini-2.5-flash", messages);
        ...
        const runFullCouncil = (userQuery: string) =>
            Effect.gen(function* () {
                ...
                const [stage2Results, labelToModel] = yield* stage2CollectRankings(userQuery, stage1Results);

                // Calculate aggregate rankings
                const aggregateRankings = calculateAggregateRankings(stage2Results, labelToModel);

                // Stage 3: Synthesize final answer
                const stage3Result = yield* stage3SynthesizeFinal(userQuery, stage1Results, stage2Results);
                ...
                return [stage1Results, stage2Results, stage3Result, metadata] as const;
            });
...
export const calculateAggregateRankings = (
    ...
    Effect.runSync(
        Effect.gen(function* () {
            const council = yield* CouncilService;
            return yield* council.calculateAggregateRankings(stage2Results, labelToModel);
        }).pipe(Effect.provide(CouncilServiceLive))
    );
...
export const stage2CollectRankings = (userQuery: string, stage1Results: Stage1Response[]) =>
    Effect.runSync(
        Effect.gen(function* () {
            const council = yield* CouncilService;
...
export const stage3SynthesizeFinal = (userQuery: string, stage1Results: Stage1Response[], stage2Results: Stage2Response[]) =>
    Effect.runSync(
        Effect.gen(function* () {
            const council = yield* CouncilService;
            return yield* council.stage3SynthesizeFinal(userQuery, stage1Results, stage2Results);
        }).pipe(Effect.provide(CouncilServiceLive))
    );